...
func RenderServiceRouter(filename string, collection *ServiceCollection) error {
	...
	generateRouterStruct(gen, collection)
	generateRouterHandler(gen, collection)
	generateRouterEndpoints(gen, collection)

	return gen.Save(filename)
}

func generateRouterStruct(gen *jen.File, c *ServiceCollection) {
...
func generateRouterHandler(gen *jen.File, c *ServiceCollection) {
...
func generateRouterEndpoints(gen *jen.File, c *ServiceCollection) {
	for _, service := range c.Services {
		for _, endpoint := range service.Endpoints {
			generateEndpointWrapper(gen, endpoint)
		}
	}
}

func generateEndpointWrapper(gen *jen.File, endpoint *Endpoint) {
